...
app.post('/api/player/:username/use', (req, res) => {
  ...
  addActivityLog(player, 'item', `Thử sử dụng vật phẩm: ${itemId || 'không rõ'} nhưng chưa có logic.`); addActivityLog(player, 'item', `Thử sử dụng vật phẩm: ${itemId || 'không rõ'} nhưng chưa có logic.`); sendPlayer(res, players, player, { message: 'Vật phẩm này chưa có logic sử dụng.' });
});
