...
router.post('/player/:id/equip',wrap(async(req,res)=>res.json({success:true,...await service.equip(req.params.id,req.body.artId,req.body.slotIndex)})));
router.post('/player/:id/unequip',wrap(async(req,res)=>res.json({success:true,...await service.unequip(req.params.id,req.body.category,req.body.slotIndex)})));
